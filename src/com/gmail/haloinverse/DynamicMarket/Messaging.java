...
/*     */{
    ...
    public static String colBracket = "&d"; // Highlighting of brackets around
                                            // params/data {PBK}
    ...
    public static String parseHighlights(String original) {
        return original.replace("{}", colNormal).replace("{CMD}", colCmd)
                .replace("{BKT}", colBracket).replace("{ERR}", colError)
                .replace("{PRM}", colParam);
    }

    public static String stripHighlights(String original) {
        return original.replace("{}", "").replace("{CMD}", "")
                .replace("{BKT}", "").replace("{ERR}", "").replace("{PRM}", "");
    }

    /*     */public static String parse(String original)
    /*     */{
        /* 70 */return parseHighlights(original).replaceAll("(&([a-z0-9]))",
                "§$2").replace("&&", "&");
        /*     */}
    ...
    /*     */{
        /* 87 */return original.replace("<black>", "§0")
 ...
 * com.nijikokun.bukkit.SimpleShop.Messaging Java Class Version: 5 (49.0)

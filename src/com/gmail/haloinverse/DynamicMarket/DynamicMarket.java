...
public class DynamicMarket extends JavaPlugin {
    ...
    private class MyServerListener extends ServerListener {
        ...
        public void onPluginEnabled(PluginEvent event) {
            ...
            String thisPluginName = event.getPlugin().getDescription()
                    .getName();
    ...
    public void onDisable() {
        log.info(Messaging.bracketize(name) + " version "
                + Messaging.bracketize(version) + " (" + codename
                + ") disabled");
    }
    ...
    public void onEnable() {
        ...
        registerEvents();
        log.info(Messaging.bracketize(name) + " version "
                + Messaging.bracketize(version) + " (" + codename + ") enabled");
    }

    private void registerEvents() {
        PluginManager thisPluginManager = getServer().getPluginManager();
        thisPluginManager.registerEvent(Event.Type.PLAYER_COMMAND_PREPROCESS,
                this.playerListener, Event.Priority.Normal, this);
        thisPluginManager.registerEvent(Event.Type.PLUGIN_ENABLE,
                this.myServerListener, Event.Priority.Monitor, this);
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd,
        ...
        if (!wrapperMode) {
            boolean thisReturn;
            thisReturn = this.playerListener.parseCommand(sender,
                    cmd.getName(), args, "", defaultShopAccount,
    ...
    public boolean wrapperCommand(CommandSender sender, String cmd,
            ...
            boolean freeAccount) {
        return this.playerListener.parseCommand(sender, cmd, args,
                (shopLabel == null ? "" : shopLabel), accountName, freeAccount);
    }

    public boolean wrapperCommand(CommandSender sender, String cmd,
            String[] args, String shopLabel) {
        return wrapperCommand(sender, cmd, args, (shopLabel == null ? ""
                : shopLabel), defaultShopAccount, defaultShopAccountFree);
    }
    ...
    public void setup() {
        Settings = new iProperty(getDataFolder() + File.separator + name
                + ".settings");

        // ItemsFile = new iProperty("items.db");
        itemsPath = Settings.getString("items-db-path", getDataFolder()
                + File.separator);
        ...
        if (this.database_type.equalsIgnoreCase("mysql"))
            db = new DatabaseMarket(DatabaseMarket.Type.MYSQL, "Market", items,
                    mysql_dbEngine, this);
        else
            db = new DatabaseMarket(DatabaseMarket.Type.SQLITE, "Market",
                    items, "", this);

        csvFileName = Settings.getString("csv-file", "shopDB.csv");
        csvFilePath = Settings.getString("csv-file-path", getDataFolder()
                + File.separator);
        ...
        Messaging.colNormal = "&"
                + Settings.getString("text-colour-normal", "e");
        Messaging.colCmd = "&" + Settings.getString("text-colour-command", "f");
        Messaging.colBracket = "&"
                + Settings.getString("text-colour-bracket", "d");
        ...
        defaultShopAccountFree = Settings.getBoolean(
                "default-shop-account-free", defaultShopAccountFree);

        transLogFile = Settings.getString("transaction-log-file", transLogFile);
        transLogAutoFlush = Settings.getBoolean("transaction-log-autoflush",
                transLogAutoFlush);
        if ((transLogFile != null) && (!transLogFile.isEmpty()))
            transLog = new TransactionLogger(this, getDataFolder()
                    + File.separator + transLogFile, transLogAutoFlush);
        else
            transLog = new TransactionLogger(this, null, false);

        String econTypeString = Settings
                .getString("economy-plugin", "iconomy3");
        if (econTypeString.equalsIgnoreCase("iconomy3"))
            ...
            log.severe(Messaging.bracketize(name)
                    + " Invalid economy setting for 'economy-plugin='.");
            econType = EconType.NONE;
        }

        // yamlPropTest = new YamlPropFile(getDataFolder() + File.separator +
        // "SimpleMarket.yml");
        // yamlPropTest.load();
    }
    ...
    public void setupPermissions() {
        if (simplePermissions) {
            Permissions = null;
            log.info(Messaging.bracketize(name)
                    + " Simple permission system active.");
        } else if (wrapperPermissions)
            log.info(Messaging.bracketize(name)
                    + " Permissions will be delegated to wrapper plugin.");
        else {
            ...
            if (Permissions == null)
                if (test != null) {
                    ...
                    log.info(Messaging.bracketize(name)
                            + " Standard Permission plugin enabled.");
                } else {
                    log.info(Messaging.bracketize(name)
    ...
    public void connectEconomy(Plugin thisPlugin) {
        ...
        /*
         * else if(econType == EconType.ANYCONOMY) { if(thisPlugin instanceof
         * AnyConomy) connectToAnyConomy((AnyConomy)thisPlugin); }
         */

    }

    /*
     * public void connectToAnyConomy(AnyConomy thisPlugin) { anyConomy =
     * thisPlugin; currency = ""; econLoaded = true;

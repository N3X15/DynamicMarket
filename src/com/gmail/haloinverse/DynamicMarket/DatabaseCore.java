...
public abstract class DatabaseCore {
    ...
    protected boolean deleteDatabase(String tableSuffix) {
        ...
        if (myQuery.isOK)
            plugin.log.info("[" + plugin.name
                    + "] Database table successfully deleted.");
        else
            plugin.log.severe("[" + plugin.name
    ...
    protected Connection connection() throws ClassNotFoundException,
        ...
        /* 56 */newConn = DriverManager.getConnection(DynamicMarket.mysql,
                DynamicMarket.mysql_user, DynamicMarket.mysql_pass);
    ...
    /*
     * { //SQLHandler myQuery = new SQLHandler(this); //if
     * (this.database.equals(Type.SQLITE)) //
     * myQuery.executeStatement("CREATE TABLE " + tableName+shopLabel +
     * " ( id INT ( 255 ) PRIMARY KEY , item INT ( 255 ) NOT NULL, type INT ( 255 ) NOT NULL, buy INT ( 255 ) NOT NULL, sell INT ( 255 ) NOT NULL, per INT ( 255 ) NOT NULL);CREATE INDEX itemIndex on balances (item);CREATE INDEX typeIndex on balances (type);CREATE INDEX buyIndex on iBalances (buy);CREATE INDEX sellIndex on iBalances (sell);CREATE INDEX perIndex on iBalances (per);"
     * ); //else // myQuery.executeStatement("CREATE TABLE " +
     * tableName+shopLabel +
     * " ( id INT( 255 ) NOT NULL AUTO_INCREMENT, item INT( 255 ) NOT NULL, type INT( 255 ) NOT NULL, buy INT( 255 ) NOT NULL, sell INT( 255 ) NOT NULL, per INT( 255 ) NOT NULL ,PRIMARY KEY ( id ), INDEX ( item, type, buy, sell, per )) ENGINE = MYISAM;"
     * ); //myQuery.close(); //return myQuery.isOK; return false; }
     */
     ...
     * myQuery.inputList.add(newItem.buy); myQuery.inputList.add(newItem.sell);
     * myQuery.inputList.add(newItem.count);
     * myQuery.prepareStatement("INSERT INTO " + tableName +
     * " (item,type,buy,sell,per) VALUES (?,?,?,?,?)"); myQuery.executeUpdate();
     * 
     * myQuery.close(); return (myQuery.isOK);
     * 
     * return false; }
     */
    ...
    /*
     * { //CHANGED: Shouldn't need to alter item subtypes in existing records.
     * New item+type records should be added instead. SQLHandler myQuery = new
     * SQLHandler(this);
     * 
     * myQuery.inputList.add(updated.buy); myQuery.inputList.add(updated.sell);
     ...
     * " SET buy = ?, sell = ?, per = ? WHERE item = ? AND type = ?" +
     * ((this.database.equals(Type.SQLITE)) ? "" : " LIMIT 1"));
     * 
    ...
    /*
     * { //CHANGED: This now spits out a list of ShopItems, instead of a list of
     * arrays of ints. //If pageNum=0, return the entire list.
     * 
     * //TODO: After incorporating items list into database, do text-matching.
     * SQLHandler myQuery = new SQLHandler(this); ArrayList<ShopItem> data = new
     * ArrayList<ShopItem>(); int startItem = 0; int numItems = 9999999; if
     * (pageNum > 0) { startItem = (pageNum - 1) * 8; numItems = 8; }
     * 
     * myQuery.inputList.add(startItem); myQuery.inputList.add(numItems);
     * myQuery.prepareStatement("SELECT * FROM " + tableName +
     * " ORDER BY item ASC, type ASC LIMIT ?, ?");
     * 
     * myQuery.executeQuery();
     * 
     * if (myQuery.isOK) try { while (myQuery.rs.next()) data.add(new
     * ShopItem(myQuery.rs.getInt("item"),myQuery.rs.getInt("type"),
     * myQuery.rs.getInt("buy"), myQuery.rs.getInt("sell"),
     * myQuery.rs.getInt("per") )); } catch (SQLException ex) {
     * logSevereException("SQL Error during ArrayList fetch: " + dbTypeString(),
     * ex); }
     * 
     * myQuery.close();
     * 
    ...
    /*
     * { //CHANGED: Returns ShopItems now. SQLHandler myQuery = new
     * SQLHandler(this); ShopItem data = null;
     * 
     * myQuery.inputList.add(thisItem.itemId);
     * myQuery.inputList.add(thisItem.subType);
     * myQuery.prepareStatement("SELECT * FROM " + tableName +
     * " WHERE item = ? AND type = ? LIMIT 1");
     * 
     * myQuery.executeQuery();
     * 
     * try { if (myQuery.rs != null) if (myQuery.rs.next()) data = new
     * ShopItem(myQuery.rs.getInt("item"), myQuery.rs.getInt("type"),
     * myQuery.rs.getInt("buy"), myQuery.rs.getInt("sell"),
     * myQuery.rs.getInt("per")); } catch (SQLException ex) {
     * logSevereException("Error retrieving shop item data with " +
     * dbTypeString(), ex); data = null; }
     * 
     * myQuery.close();
     * 
     * if (data == null) { data = new ShopItem(); } return data; }
     */
 ...
 * com.nijikokun.bukkit.SimpleShop.Database Java Class Version: 5 (49.0) JD-Core
 * Version: 0.5.3
 */